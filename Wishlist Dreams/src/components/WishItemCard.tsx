      ...
      <Card className="border-primary/30 bg-card">
        <CardContent className="p-4 flex flex-col gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={editName} onChange={(e) => { setEditName(e.target.value); if (nameError) setNameError(''); }} className="bg-secondary border-border text-foreground h-9 text-sm" />
            {nameError && <p className="text-xs text-destructive">{nameError}</p>}
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-foreground">Link</Label>
            <Input value={editLink} onChange={(e) => setEditLink(e.target.value)} placeholder="https://..." className="bg-secondary border-border text-foreground h-9 text-sm" />
          </div>
          <div className="space-y-1.5">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={editSizeColor} onChange={(e) => setEditSizeColor(e.target.value)} className="bg-secondary border-border text-foreground h-9 text-sm" />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-foreground">📝 Observações</Label>
            <textarea value={editNotes} onChange={(e) => setEditNotes(e.target.value)} rows={2} className="flex w-full rounded-md border bg-secondary border-border px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" />
          </div>
    ...
    <Card className={`border-border bg-card group transition-colors ${purchased ? 'opacity-60' : 'hover:border-primary/30'}`}>
      <CardContent className="p-4 flex flex-col gap-3">
        <div className="flex items-start justify-between gap-2">
          <p className={`font-medium leading-snug ${purchased ? 'line-through text-muted-foreground' : 'text-foreground'}`}>
            {item.name}
          </p>
          {isOwner && (
        ...
        <div className="flex items-center justify-between gap-2 mt-auto">
          ...
          {item.link && (
            <a
              ...
              className="inline-flex items-center gap-1.5 text-xs font-medium text-primary hover:text-accent bg-primary/10 hover:bg-primary/20 px-3 py-1.5 rounded-lg transition-colors"
            >
          ...
          <div className="pt-1 border-t border-border">
            {!purchased ? (
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-1.5 text-xs font-medium text-primary hover:bg-primary/10 py-2 rounded-lg transition-colors"
              >
              ...
              <button
                ...
                className="w-full inline-flex items-center justify-center gap-1.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-secondary py-2 rounded-lg transition-colors"
              >
